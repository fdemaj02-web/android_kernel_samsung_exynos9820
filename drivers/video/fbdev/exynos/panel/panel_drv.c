...
int panel_update_dim_type(struct panel_device *panel, u32 dim_type)
{
	...
	if (dim_type == DIM_TYPE_DIM_FLASH) {
		...
		/*
		 * Checksum-only validation can false-positive on a blank or
		 * mismatched replacement panel: an all-zero calibration
		 * buffer paired with a zero checksum passes the checksum
		 * comparison above even though it is not real per-unit data.
		 * Explicitly reject an all-zero live MTP read here so we
		 * don't trust bogus flash gamma tables (this is what produces
		 * a shadow/low-gray-level color cast) and instead fall back
		 * to the algorithmic AID dimming path below.
		 */
		if (state != GAMMA_FLASH_ERROR_READ_FAIL) {
			int __i;
			bool mtp_all_zero = true;

			for (__i = 0; __i < sizeof(result->mtp_reg); __i++) {
				if (result->mtp_reg[__i] != 0) {
					mtp_all_zero = false;
					break;
				}
			}
			...
			if (mtp_all_zero) {
				pr_err("%s, live MTP read is all-zero (no factory calibration) "
						"- refusing dim_flash gamma, falling back to AID dimming\n",
						__func__);
				if (!state)
