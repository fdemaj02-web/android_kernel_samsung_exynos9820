		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (nit_min < 0 || nit_max < nit_min || nit_max > 500 ||
			tp_min < 0 || tp_max < tp_min ||
			r < -40 || r > 40 || g < -40 || g > 40 || b < -40 || b > 40) {
		panel_err("PANEL:ERR:%s: out of range - nit 0-500, tp 0+, r/g/b -40..40, go smaller and iterate\n",
				__func__);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (nit_min < 0 || nit_max < nit_min || nit_max > 500 ||
			tp_min < 0 || tp_max < tp_min ||
			delta < -30 || delta > 30) {
		panel_err("PANEL:ERR:%s: out of range - nit 0-500, tp 0+, delta -30..30, go smaller and iterate\n",
				__func__);
		return -EINVAL;
	}
