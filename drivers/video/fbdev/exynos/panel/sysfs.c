...
/* Must match MANUAL_OFFSET_MAX_TP in s6e3ha9.c - separate compilation
 * unit, no shared header for this, so the value is duplicated here
 * on purpose. If you change one, change both. */
...
extern int manual_offset_color(struct panel_device *panel, int id,
		int nit_min, int nit_max, int tp_min, int tp_max,
		int dr, int dg, int db);
extern int manual_offset_gray(struct panel_device *panel, int id,
		int nit_min, int nit_max, int tp_min, int tp_max, int delta);
extern int manual_offset_reset(struct panel_device *panel, int id);
extern int manual_offset_gray_array(struct panel_device *panel, int id,
		int nit_min, int nit_max, s32 *deltas, int nr_deltas);
extern int manual_offset_color_array(struct panel_device *panel, int id,
		int nit_min, int nit_max, s32 *rgb, int nr_rgb);
extern int manual_offset_irc_set(s32 *deltas, int nr_deltas);
extern int manual_offset_irc_reset(void);
extern int manual_offset_status_lut_store_impl(struct panel_device *panel, int id, int lut);
extern int manual_offset_status_show_impl(struct panel_device *panel, int id, char *buf, size_t bufsize);
extern int manual_offset_tp_cal(struct panel_device *panel, int id, int tp_idx, int dr, int dg, int db);
extern int manual_offset_vt(struct panel_device *panel, int id, int vt_idx, int delta);
extern int manual_offset_tpvt_reset(struct panel_device *panel, int id);
extern int manual_offset_tpvt_status_show_impl(struct panel_device *panel, int id, char *buf, size_t bufsize);

/*
 * manual_offset - live RGB color-balance offset for testing after a
 * panel replacement. No pre-baked correction data - you supply
 * deltas, look at the real screen, iterate.
 *
 * Two independent axes:
 *   NIT_MIN/NIT_MAX  - which SCREEN BRIGHTNESS level's curve to touch
 *   TP_MIN/TP_MAX    - which GRAY TONE within that curve (0=black,
 *                       11=white on this panel - shadows are low
 *                       numbers, highlights are high numbers)
 *
 * Usage (all tap points, old form):   echo "NIT_MIN NIT_MAX R G B" > manual_offset
 * Usage (specific tap points):        echo "NIT_MIN NIT_MAX TP_MIN TP_MAX R G B" > manual_offset
 * Reset:                              echo "reset" > manual_offset  (shared with manual_offset_gray)
 * Example (whole curve):              echo "0 40 -3 0 -2" > manual_offset
 * Example (just tap points 3-7,       echo "0 500 3 7 -6 3 3" > manual_offset
 *   the shadow-to-midtone transition,
 *   across every brightness level):
 */
static ssize_t manual_offset_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"NIT_MIN NIT_MAX R G B\" > manual_offset (all tap points)\n"
		"echo \"NIT_MIN NIT_MAX TP_MIN TP_MAX R G B\" > manual_offset (specific tones)\n"
		"echo \"reset\" > manual_offset  (restore factory, both nodes)\n");
}

static ssize_t manual_offset_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int nit_min, nit_max, tp_min, tp_max, r, g, b;
	int ret;
	...
	if (strncmp(buf, "reset", 5) == 0) {
		ret = manual_offset_reset(panel, PANEL_BL_SUBDEV_TYPE_DISP);
		return ret < 0 ? ret : size;
	}

	/* try the 7-arg tap-point-targeted form first */
	ret = sscanf(buf, "%d %d %d %d %d %d %d",
			&nit_min, &nit_max, &tp_min, &tp_max, &r, &g, &b);
	if (ret != 7) {
		/* fall back to the 5-arg "all tap points" form */
		ret = sscanf(buf, "%d %d %d %d %d", &nit_min, &nit_max, &r, &g, &b);
		if (ret != 5) {
			panel_err("PANEL:ERR:%s: need \"NIT_MIN NIT_MAX R G B\", "
					"\"NIT_MIN NIT_MAX TP_MIN TP_MAX R G B\", or \"reset\"\n",
					__func__);
			return -EINVAL;
		}
		tp_min = 0;
		tp_max = 999; /* clamped to nr_tp-1 inside manual_offset_color */
	}

	if (nit_min < 0 || nit_max < nit_min ||
			tp_min < 0 || tp_max < tp_min) {
		panel_err("PANEL:ERR:%s: invalid range - nit_min/nit_max/tp_min/tp_max must be sane\n",
				__func__);
		return -EINVAL;
	}

	ret = manual_offset_color(panel, PANEL_BL_SUBDEV_TYPE_DISP,
			nit_min, nit_max, tp_min, tp_max, r, g, b);
	return ret < 0 ? ret : size;
}

/*
 * manual_offset_gray - live grayscale darkness offset. Negative =
 * darker at that tone, positive = lighter. Same two-axis idea as
 * manual_offset: NIT_MIN/NIT_MAX picks brightness level, TP_MIN/
 * TP_MAX picks which gray tone (0=black..11=white on this panel).
 *
 * Usage (all tap points, old form):  echo "NIT_MIN NIT_MAX DELTA" > manual_offset_gray
 * Usage (specific tap points):       echo "NIT_MIN NIT_MAX TP_MIN TP_MAX DELTA" > manual_offset_gray
 * Reset:                             echo "reset" > manual_offset_gray  (shared with manual_offset)
 * Example (lighten tap points 5-8,   echo "0 500 5 8 5" > manual_offset_gray
 *   light-shadow band, all brightness):
 */
static ssize_t manual_offset_gray_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"NIT_MIN NIT_MAX DELTA\" > manual_offset_gray (all tap points, negative=darker)\n"
		"echo \"NIT_MIN NIT_MAX TP_MIN TP_MAX DELTA\" > manual_offset_gray (specific tones)\n"
		"echo \"reset\" > manual_offset_gray  (restore factory, both nodes)\n");
}

static ssize_t manual_offset_gray_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int nit_min, nit_max, tp_min, tp_max, delta;
	int ret;
	...
	/* try the 5-arg tap-point-targeted form first */
	ret = sscanf(buf, "%d %d %d %d %d", &nit_min, &nit_max, &tp_min, &tp_max, &delta);
	if (ret != 5) {
		/* fall back to the 3-arg "all tap points" form */
		ret = sscanf(buf, "%d %d %d", &nit_min, &nit_max, &delta);
		if (ret != 3) {
			panel_err("PANEL:ERR:%s: need \"NIT_MIN NIT_MAX DELTA\", "
					"\"NIT_MIN NIT_MAX TP_MIN TP_MAX DELTA\", or \"reset\"\n",
					__func__);
			return -EINVAL;
		}
		tp_min = 0;
		tp_max = 999; /* clamped to nr_tp-1 inside manual_offset_gray */
	}
	...
	ret = manual_offset_gray(panel, PANEL_BL_SUBDEV_TYPE_DISP,
			nit_min, nit_max, tp_min, tp_max, delta);
	return ret < 0 ? ret : size;
}

/*
 * manual_offset_gray_array - set ALL tap points explicitly in one
 * command instead of one uniform delta. Index 0 = deepest black.
 * SETS the absolute value (not additive) - safe to re-run.
 * Usage: echo "NIT_MIN NIT_MAX D0 D1 D2 D3 D4 D5 D6 D7 D8 D9 D10 D11" > manual_offset_gray_array
 * No numeric limit on the deltas - go as far as needed, iterate by eye.
 * Reset:  echo "reset" > manual_offset_gray  (shared baseline)
 */
static ssize_t manual_offset_gray_array_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"NIT_MIN NIT_MAX D0 D1 ... D11\" > manual_offset_gray_array "
		"(one delta per tap point, index 0 = black, sets absolute value)\n"
		"echo \"reset\" > manual_offset_gray  (restore factory, shared with manual_offset)\n");
}

static ssize_t manual_offset_status_lut_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo LUT_INDEX > manual_offset_status_lut, then cat manual_offset_status\n"
		"(0 = dimmest brightness table; check nr_lum in manual_offset_status output for the max)\n");
}

static ssize_t manual_offset_tp_cal_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"TP_INDEX DR DG DB\" > manual_offset_tp_cal  "
		"(real per-tap MTP calibration offset, additive, touches ALL brightness levels at once)\n"
		"echo \"reset\" > manual_offset_tp_cal  (restore real factory calibration, shared with manual_offset_vt)\n");
}

static ssize_t manual_offset_tp_cal_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int tp_idx, dr, dg, db, ret;

	if (panel == NULL)
		return -EINVAL;

	if (strncmp(buf, "reset", 5) == 0) {
		ret = manual_offset_tpvt_reset(panel, PANEL_BL_SUBDEV_TYPE_DISP);
		return ret < 0 ? ret : size;
	}

	ret = sscanf(buf, "%d %d %d %d", &tp_idx, &dr, &dg, &db);
	if (ret != 4) {
		panel_err("PANEL:ERR:%s: need \"TP_INDEX DR DG DB\" or \"reset\"\n", __func__);
		return -EINVAL;
	}

	ret = manual_offset_tp_cal(panel, PANEL_BL_SUBDEV_TYPE_DISP, tp_idx, dr, dg, db);
	return ret < 0 ? ret : size;
}

static ssize_t manual_offset_vt_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"VT_INDEX DELTA\" > manual_offset_vt  "
		"(real black-point voltage reference, additive, VT_INDEX 0-15)\n"
		"echo \"reset\" > manual_offset_vt  (restore real factory calibration, shared with manual_offset_tp_cal)\n");
}

static ssize_t manual_offset_vt_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int vt_idx, delta, ret;
	...
	ret = sscanf(buf, "%d %d", &vt_idx, &delta);
	if (ret != 2) {
		panel_err("PANEL:ERR:%s: need \"VT_INDEX DELTA\" or \"reset\"\n", __func__);
		return -EINVAL;
	}

	ret = manual_offset_vt(panel, PANEL_BL_SUBDEV_TYPE_DISP, vt_idx, delta);
	return ret < 0 ? ret : size;
}

static ssize_t manual_offset_tpvt_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int ret;
	...
	ret = manual_offset_tpvt_status_show_impl(panel, PANEL_BL_SUBDEV_TYPE_DISP, buf, PAGE_SIZE);
	return ret;
}

static ssize_t manual_offset_status_lut_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int lut, ret;
	...
	ret = kstrtoint(buf, 10, &lut);
	if (ret < 0)
		return ret;

	ret = manual_offset_status_lut_store_impl(panel, PANEL_BL_SUBDEV_TYPE_DISP, lut);
	if (ret < 0) {
		panel_err("PANEL:ERR:%s: invalid lut %d\n", __func__, lut);
		return ret;
	}
	return size;
}

static ssize_t manual_offset_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int ret;
	...
	ret = manual_offset_status_show_impl(panel, PANEL_BL_SUBDEV_TYPE_DISP, buf, PAGE_SIZE);
	if (ret < 0)
		return ret;
	return ret;
}

static ssize_t manual_offset_gray_array_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int nit_min, nit_max, ret, n, consumed;
	s32 deltas[MANUAL_OFFSET_MAX_TP];
	const char *p = buf;
	...
	ret = sscanf(p, "%d %d%n", &nit_min, &nit_max, &consumed);
	if (ret != 2) {
		panel_err("PANEL:ERR:%s: need \"NIT_MIN NIT_MAX D0..D11\"\n", __func__);
		return -EINVAL;
	}
	p += consumed;

	for (n = 0; n < MANUAL_OFFSET_MAX_TP; n++) {
		int v, adv;
		ret = sscanf(p, "%d%n", &v, &adv);
		if (ret != 1)
			break;
		deltas[n] = v;
		p += adv;
	...
	if (n == 0) {
		panel_err("PANEL:ERR:%s: no per-tap deltas given\n", __func__);
		return -EINVAL;
	}

	if (nit_min < 0 || nit_max < nit_min) {
		panel_err("PANEL:ERR:%s: invalid nit range\n", __func__);
		return -EINVAL;
	}

	ret = manual_offset_gray_array(panel, PANEL_BL_SUBDEV_TYPE_DISP,
			nit_min, nit_max, deltas, n);
	return ret < 0 ? ret : size;
}

/*
 * manual_offset_color_array - set ALL tap points' R,G,B explicitly in
 * one command. Index 0 = deepest black. SETS the absolute value.
 * Usage: echo "NIT_MIN NIT_MAX R0 G0 B0 R1 G1 B1 ... R11 G11 B11" > manual_offset_color_array
 * No numeric limit - go as far as needed, iterate by eye.
 * Reset:  echo "reset" > manual_offset  (shared baseline)
 */
static ssize_t manual_offset_color_array_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"NIT_MIN NIT_MAX R0 G0 B0 ... R11 G11 B11\" > manual_offset_color_array "
		"(RGB triplet per tap point, index 0 = black, sets absolute value)\n"
		"echo \"reset\" > manual_offset  (restore factory, shared with manual_offset_gray)\n");
}

static ssize_t manual_offset_color_array_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int nit_min, nit_max, ret, n, consumed;
	s32 rgb[MANUAL_OFFSET_MAX_TP * 3];
	const char *p = buf;
	...
	ret = sscanf(p, "%d %d%n", &nit_min, &nit_max, &consumed);
	if (ret != 2) {
		panel_err("PANEL:ERR:%s: need \"NIT_MIN NIT_MAX R0 G0 B0..R11 G11 B11\"\n", __func__);
		return -EINVAL;
	}
	p += consumed;

	for (n = 0; n < MANUAL_OFFSET_MAX_TP * 3; n++) {
		int v, adv;
		ret = sscanf(p, "%d%n", &v, &adv);
		if (ret != 1)
			break;
	...
	if (n == 0 || (n % 3) != 0) {
		panel_err("PANEL:ERR:%s: values must come in complete R G B triplets\n", __func__);
		return -EINVAL;
	}
	...
	ret = manual_offset_color_array(panel, PANEL_BL_SUBDEV_TYPE_DISP,
			nit_min, nit_max, rgb, n);
	return ret < 0 ? ret : size;
}

/*
 * manual_offset_irc - persistent per-tap delta applied to the IRC
 * (low-brightness/shadow compensation) table, re-applied every time
 * brightness changes (unlike a raw one-time edit which would get
 * wiped out on the next brightness step). Index 0 = deepest shadow
 * tap point. No numeric limit - output is clamped to a valid byte
 * (0-255) internally so it can't wrap/corrupt, but the delta itself
 * is unrestricted.
 * Usage: echo "D0 D1 D2 ... D11" > manual_offset_irc
 * Reset: echo "reset" > manual_offset_irc
 */
static ssize_t manual_offset_irc_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"D0 D1 ... D11\" > manual_offset_irc "
		"(one delta per IRC tap point, index 0 = deepest shadow)\n"
		"echo \"reset\" > manual_offset_irc  (clear IRC deltas only)\n");
}

static ssize_t manual_offset_irc_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int ret, n;
	s32 deltas[MANUAL_OFFSET_MAX_TP];
	...
	if (n == 0) {
		panel_err("PANEL:ERR:%s: need \"D0 D1 ... D11\" or \"reset\"\n", __func__);
		return -EINVAL;
	}

	ret = manual_offset_irc_set(deltas, n);
	return ret < 0 ? ret : size;
}

/*
 * mtp_diag - READ-ONLY diagnostic. Shows the raw per-unit factory
 * trim data (MTP) actually read from THIS specific panel. If this
 * comes back all zeros, it means this panel has no individual
 * calibration trim - it's running on the generic reference curve
 * alone, which would explain a persistent color cast that generic
 * per-tap offsets can approximate but never fully cancel.
 * Changes nothing on the display - for diagnosis only.
 */
static ssize_t mtp_diag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	struct panel_info *panel_data;
	struct resinfo *res;
	int i, len = 0, nonzero_count = 0;
	...
	if (!res) {
		return snprintf(buf, PAGE_SIZE, "mtp resource not found\n");
	}

	len += snprintf(buf + len, PAGE_SIZE - len, "mtp raw bytes (%d total): ", res->dlen);
	for (i = 0; i < res->dlen && len < PAGE_SIZE - 8; i++) {
		len += snprintf(buf + len, PAGE_SIZE - len, "%02X ", res->data[i]);
		if (res->data[i] != 0)
			nonzero_count++;
	}
	len += snprintf(buf + len, PAGE_SIZE - len,
			"\nnonzero bytes: %d of %d\n", nonzero_count, res->dlen);
...
#endif
	__PANEL_ATTR_RW(manual_offset, 0664),
	__PANEL_ATTR_RW(manual_offset_gray, 0664),
	__PANEL_ATTR_RW(manual_offset_gray_array, 0664),
	__PANEL_ATTR_RW(manual_offset_status_lut, 0664),
	__PANEL_ATTR_RO(manual_offset_status, 0444),
	__PANEL_ATTR_RW(manual_offset_tp_cal, 0664),
	__PANEL_ATTR_RW(manual_offset_vt, 0664),
	__PANEL_ATTR_RO(manual_offset_tpvt_status, 0444),
	__PANEL_ATTR_RW(manual_offset_color_array, 0664),
	__PANEL_ATTR_RW(manual_offset_irc, 0664),
	__PANEL_ATTR_RO(mtp_diag, 0444),
#ifdef CONFIG_SUPPORT_GRAM_CHECKSUM
