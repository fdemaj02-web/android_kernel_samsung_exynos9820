...
extern int manual_offset_gray(struct panel_device *panel, int id,
		int nit_min, int nit_max, int delta);
 ...
 * darker at that tap point. Same honest-tool rules as manual_offset.
 *
 * Usage:   echo "NIT_MIN NIT_MAX DELTA" > manual_offset_gray
 * Reset:   echo "reset" > manual_offset_gray   (shared with manual_offset)
 * Example: echo "0 40 -5" > manual_offset_gray
 */
static ssize_t manual_offset_gray_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"NIT_MIN NIT_MAX DELTA\" > manual_offset_gray  (negative = darker)\n"
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret != 3) {
		panel_err("PANEL:ERR:%s: need \"NIT_MIN NIT_MAX DELTA\" or \"reset\"\n", __func__);
		return -EINVAL;
	}

	if (nit_min < 0 || nit_max < nit_min || nit_max > 500 ||
			delta < -30 || delta > 30) {
		panel_err("PANEL:ERR:%s: out of range - nit 0-500, delta -30..30, go smaller and iterate\n",
				__func__);
		return -EINVAL;
	}

	ret = manual_offset_gray(panel, PANEL_BL_SUBDEV_TYPE_DISP,
			nit_min, nit_max, delta);
	return ret < 0 ? ret : size;
}
