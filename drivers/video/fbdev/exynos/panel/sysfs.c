 ...
 *
 * Usage:   echo "NIT_MIN NIT_MAX R G B" > manual_offset
 * Reset:   echo "reset" > manual_offset   (shared with manual_offset_gray)
 * Example: echo "0 40 -3 0 -2" > manual_offset
 */
static ssize_t manual_offset_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
		"echo \"NIT_MIN NIT_MAX R G B\" > manual_offset\n"
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret != 5) {
		panel_err("PANEL:ERR:%s: need \"NIT_MIN NIT_MAX R G B\" or \"reset\"\n", __func__);
		return -EINVAL;
	}

	if (nit_min < 0 || nit_max < nit_min || nit_max > 500 ||
			r < -40 || r > 40 || g < -40 || g > 40 || b < -40 || b > 40) {
		panel_err("PANEL:ERR:%s: out of range - nit 0-500, r/g/b -40..40, go smaller and iterate\n",
				__func__);
		return -EINVAL;
	}

	ret = manual_offset_color(panel, PANEL_BL_SUBDEV_TYPE_DISP,
			nit_min, nit_max, r, g, b);
	return ret < 0 ? ret : size;
}
