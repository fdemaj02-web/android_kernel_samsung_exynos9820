...
/*
 * shadowRGB - live-adjustable shadow/dark-tone color correction.
 * Unlike whiteRGB (which shifts the whole brightness range),
 * this only touches luminance levels at/below the threshold,
 * leaving your midtones and highlights untouched.
 *
 * Usage: echo "R G B [threshold_nit]" > shadowRGB
 * Example: echo "-15 0 -8 60" > shadowRGB
 *   (reduce red by 15, leave green alone, reduce blue by 8,
 *    only for luminance <= 60 nit)
 * If threshold is omitted, defaults to 60 nit.
 */
static int last_shadow_r, last_shadow_g, last_shadow_b, last_shadow_threshold = 60;

/* declared here since s6e3ha9.c may not be included generically in
 * this shared sysfs.c - this build always targets s6e3ha9 (S10+)
 */
extern int s6e3ha9_refresh_gamma_with_shadow_correction(struct panel_device *panel,
		int id, int shadow_r, int shadow_g, int shadow_b,
		int luminance_threshold);

static ssize_t shadowRGB_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d %d %d %d\n",
			last_shadow_r, last_shadow_g, last_shadow_b,
			last_shadow_threshold);
}

/*
 * precision_shadow_fix - one simple dial (0-100) that applies the
 * REAL per-tap correction measured from this panel's own factory
 * data, instead of one flat number across all tap points.
 *
 * Usage: echo "STRENGTH THRESHOLD" > precision_shadow_fix
 * Example: echo "50 60" > precision_shadow_fix
 *   (apply 50% of the full measured correction, below 60 nit)
 * Start around 30-50 and increase gradually.
 */
static int last_precision_strength, last_precision_threshold = 60;

extern int s6e3ha9_apply_precision_shadow_fix(struct panel_device *panel,
		int id, int strength_percent, int luminance_threshold);

static ssize_t precision_shadow_fix_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d %d\n",
			last_precision_strength, last_precision_threshold);
}

static ssize_t precision_shadow_fix_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int strength, threshold = 60;
	int ret;
	...
	ret = sscanf(buf, "%d %d", &strength, &threshold);
	if (ret < 1) {
		panel_err("PANEL:ERR:%s: need at least a strength value\n", __func__);
		return -EINVAL;
	}

	if (strength < 0 || strength > 100) {
		panel_err("PANEL:ERR:%s: strength must be 0-100\n", __func__);
		return -EINVAL;
	}

	/* undo previous correction first so repeated writes don't stack */
	if (last_precision_strength != 0) {
		s6e3ha9_apply_precision_shadow_fix(panel,
				PANEL_BL_SUBDEV_TYPE_DISP,
				-last_precision_strength, last_precision_threshold);
	}

	ret = s6e3ha9_apply_precision_shadow_fix(panel,
			PANEL_BL_SUBDEV_TYPE_DISP, strength, threshold);
	if (ret < 0) {
		panel_err("PANEL:ERR:%s: failed (%d)\n", __func__, ret);
		return ret;
	}

	last_precision_strength = strength;
	last_precision_threshold = threshold;

	return size;
}

/*
 * color_fix - applies real measured correction across your ENTIRE
 * brightness range (shadows through full brightness), each level
 * matched to its own closest real Samsung factory data point.
 * No threshold needed - it's accurate everywhere on its own.
 *
 * Usage: echo "STRENGTH" > color_fix   (0-100)
 * Start around 40-50, increase if not enough.
 */
static int last_color_fix_strength;

extern int s6e3ha9_apply_full_range_color_fix(struct panel_device *panel,
		int id, int strength_percent);

static ssize_t color_fix_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", last_color_fix_strength);
}

static ssize_t color_fix_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int strength;
	...
	ret = sscanf(buf, "%d", &strength);
	if (ret < 1 || strength < 0 || strength > 100) {
		panel_err("PANEL:ERR:%s: strength must be 0-100\n", __func__);
		return -EINVAL;
	}

	if (last_color_fix_strength != 0) {
		s6e3ha9_apply_full_range_color_fix(panel,
				PANEL_BL_SUBDEV_TYPE_DISP, -last_color_fix_strength);
	}

	ret = s6e3ha9_apply_full_range_color_fix(panel,
			PANEL_BL_SUBDEV_TYPE_DISP, strength);
	if (ret < 0) {
		panel_err("PANEL:ERR:%s: failed (%d)\n", __func__, ret);
		return ret;
	}

	last_color_fix_strength = strength;

	return size;
}

/*
 * manual_gamma - true manual R/G/B control, any direction, no
 * formula. Three independent zones so you can tune dark, mid,
 * and bright separately.
 *
 * Usage: echo "ZONE R G B" > manual_gamma
 * ZONE: 0=dark(<=60nit) 1=mid(60-172nit) 2=bright(>172nit)
 * Example: echo "0 -20 5 -15" > manual_gamma
 *   (dark zone: reduce red 20, boost green 5, reduce blue 15)
 * Each write is ADDITIVE on top of current state for that zone -
 * write "0 0 0 0" to reset a zone back to neutral.
 */
static int last_manual_gamma[3][3]; /* [zone][R,G,B], cumulative */

extern int s6e3ha9_apply_manual_gamma(struct panel_device *panel,
		int id, int zone, int r, int g, int b);

static ssize_t manual_gamma_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE,
			"dark:   %d %d %d\nmid:    %d %d %d\nbright: %d %d %d\n",
			last_manual_gamma[0][0], last_manual_gamma[0][1], last_manual_gamma[0][2],
			last_manual_gamma[1][0], last_manual_gamma[1][1], last_manual_gamma[1][2],
			last_manual_gamma[2][0], last_manual_gamma[2][1], last_manual_gamma[2][2]);
}

static ssize_t manual_gamma_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int zone, r, g, b;
	...
	ret = sscanf(buf, "%d %d %d %d", &zone, &r, &g, &b);
	if (ret != 4) {
		panel_err("PANEL:ERR:%s: need ZONE R G B\n", __func__);
		return -EINVAL;
	}

	if (zone < 0 || zone > 2) {
		panel_err("PANEL:ERR:%s: zone must be 0, 1, or 2\n", __func__);
		return -EINVAL;
	}

	if (r < -80 || r > 80 || g < -80 || g > 80 || b < -80 || b > 80) {
		panel_err("PANEL:ERR:%s: values must be -80 to 80\n", __func__);
		return -EINVAL;
	}

	ret = s6e3ha9_apply_manual_gamma(panel,
			PANEL_BL_SUBDEV_TYPE_DISP, zone, r, g, b);
	if (ret < 0) {
		panel_err("PANEL:ERR:%s: failed (%d)\n", __func__, ret);
		return ret;
	}

	last_manual_gamma[zone][0] += r;
	last_manual_gamma[zone][1] += g;
	last_manual_gamma[zone][2] += b;
...
static ssize_t shadowRGB_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int r, g, b, threshold = 60;
	int ret;
	...
	ret = sscanf(buf, "%d %d %d %d", &r, &g, &b, &threshold);
	if (ret < 3) {
		panel_err("PANEL:ERR:%s: need at least R G B\n", __func__);
		return -EINVAL;
	}

	/* keep corrections in a sane, reversible range */
	if (r < -60 || r > 60 || g < -60 || g > 60 || b < -60 || b > 60) {
		panel_err("PANEL:ERR:%s: value out of range (-60..60)\n", __func__);
		return -EINVAL;
	}

	/* undo the previous correction before applying the new one,
	 * so repeated writes don't stack on top of each other
	 */
	if (last_shadow_r || last_shadow_g || last_shadow_b) {
		s6e3ha9_refresh_gamma_with_shadow_correction(panel,
				PANEL_BL_SUBDEV_TYPE_DISP,
				-last_shadow_r, -last_shadow_g, -last_shadow_b,
				last_shadow_threshold);
	}

	ret = s6e3ha9_refresh_gamma_with_shadow_correction(panel,
			PANEL_BL_SUBDEV_TYPE_DISP, r, g, b, threshold);
	if (ret < 0) {
		panel_err("PANEL:ERR:%s: failed to apply correction (%d)\n",
				__func__, ret);
		return ret;
	}
	...
	last_shadow_g = g;
	last_shadow_b = b;
	last_shadow_threshold = threshold;
...
#endif
	__PANEL_ATTR_RW(shadowRGB, 0664),
	__PANEL_ATTR_RW(precision_shadow_fix, 0664),
	__PANEL_ATTR_RW(color_fix, 0664),
	__PANEL_ATTR_RW(manual_gamma, 0664),
