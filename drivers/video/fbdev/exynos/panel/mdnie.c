...
static int mdnie_get_coordinate(struct mdnie_info *mdnie, int *x, int *y)
{
	...
	*y = g_coord_y;
	...
	if (*x < MIN_WCRD_X || *x > MAX_WCRD_X ||
		...
		/*
		 * No valid factory MTP white-point calibration exists on this
		 * panel (raw coordinate read returns 0,0 or garbage). The
		 * arithmetic midpoint of the valid MIN/MAX range is NOT a
		 * neutral value here - mdnie_coordinate_tune_rgb() only skips
		 * correction (tune_x = tune_y = 0) when the coordinate falls
		 * within a small radius of (cal_x_center, cal_y_center); any
		 * other coordinate is run through a LUT-based interpolation
		 * that can produce a large, unbalanced correction. Fall back
		 * to the panel's own zero-correction reference point instead,
		 * so no white-point correction is applied at all - safer than
		 * guessing at a "neutral-looking" coordinate.
		 */
		*x = mdnie->props.cal_x_center;
		*y = mdnie->props.cal_y_center;
		pr_warn("%s, falling back to zero-correction coord_x:%d coord_y:%d\n",
				__func__, *x, *y);
	}
