...
static int mdnie_get_coordinate(struct mdnie_info *mdnie, int *x, int *y)
{
	...
	*y = g_coord_y;
	...
	if (*x < MIN_WCRD_X || *x > MAX_WCRD_X ||
		 ...
		 * panel (raw coordinate read returns 0,0 or garbage). Rather
		 * than feeding an out-of-range coordinate into the white-point
		 * interpolation (which produces a warm/pink cast), fall back
		 * to the midpoint of the panel's valid calibration range as a
		 * neutral reference target.
		 */
		*x = (MIN_WCRD_X + MAX_WCRD_X) / 2;
		*y = (MIN_WCRD_Y + MAX_WCRD_Y) / 2;
		pr_warn("%s, falling back to default coord_x:%d coord_y:%d\n",
				__func__, *x, *y);
	}
