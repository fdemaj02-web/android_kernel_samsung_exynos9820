 ...
 * every tap point in [nit_min, nit_max]. Negative red = less red,
 * negative blue = less blue, etc.
 */
int manual_offset_color(struct panel_device *panel, int id,
		int nit_min, int nit_max, int dr, int dg, int db)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int i, v, lum, ret;
	...
	panel_info("%s applied r:%+d g:%+d b:%+d to %d-%dnit\n",
			__func__, dr, dg, db, nit_min, nit_max);
	return 0;
}
