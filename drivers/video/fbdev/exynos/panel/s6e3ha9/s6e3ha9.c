...
/*
 * s6e3ha9_refresh_gamma_with_shadow_correction
 *
 * Applies a user-supplied RGB correction to the panel's gamma
 * calibration data, but ONLY for luminance (nit) levels at or
 * below luminance_threshold - i.e. shadows/dark UI, not the
 * whole brightness range. Then rebuilds the pre-computed gamma
 * table and forces a live brightness refresh so the change is
 * visible immediately, without a reboot.
 *
 * shadow_r/g/b: correction values, roughly -30 to +30 is a sane
 *               starting range. Negative = less of that color.
 * luminance_threshold: nit level below which the correction
 *               applies (e.g. 60 = affects shadows/dark UI only).
 */
int s6e3ha9_refresh_gamma_with_shadow_correction(struct panel_device *panel,
		...
		int luminance_threshold)
{
	...
	/* Apply correction only to luminance entries at/below threshold */
	for (i = 0; i < nr_luminance; i++) {
		if (dim_info->dim_lut[i].luminance > luminance_threshold)
			continue;
		for (v = 0; v < dim_info->nr_tp; v++) {
			dim_info->dim_lut[i].rgb_color_offset[v][RED] += shadow_r;
			dim_info->dim_lut[i].rgb_color_offset[v][GREEN] += shadow_g;
			dim_info->dim_lut[i].rgb_color_offset[v][BLUE] += shadow_b;
		}
	}

	/* Recompute the full gamma table using the corrected offsets */
	generate_gamma_table(dim_info);

	/* Rebuild the pre-computed per-luminance register rows the
	 * panel actually reads from at brightness-change time
	 */
	for (i = 0; i < nr_luminance; i++)
		get_dimming_gamma(dim_info, brt_tbl->lum[i],
				(u8 *)&gamma_maptbl->arr[i * sizeof_row(gamma_maptbl)],
	...
	/* Force a live re-push of gamma to the panel, reusing the
	 * phone's own normal brightness-update code path
	 */
	panel_bl_set_brightness(panel_bl, id, 1);

	panel_info("%s applied shadow correction r:%d g:%d b:%d below %dnit\n",
			__func__, shadow_r, shadow_g, shadow_b, luminance_threshold);

	return 0;
}
EXPORT_SYMBOL(s6e3ha9_refresh_gamma_with_shadow_correction);

/*
 * s6e3ha9_apply_precision_shadow_fix
 *
 * Unlike the flat shadowRGB correction, this applies a DIFFERENT
 * correction at each of the 12 gamma calibration points, derived
 * from this panel's own real factory ctbl data (measured, not
 * guessed) - the exact offset needed at each point to bring Red
 * and Blue into gray-balance with Green.
 *
 * strength_percent: 0-100, scales the correction. 100 = full
 *   correction as measured. Start lower (30-50) and increase.
 * luminance_threshold: nit level below which this applies.
 */
static const int precision_r_correction[12] = {
	0, 0, 0, -9, 8, 21, 25, 19, 16, 7, 3, 9
};
static const int precision_b_correction[12] = {
	0, 0, 0, 6, 12, 15, 19, 12, 9, 4, 2, 6
};

int s6e3ha9_apply_precision_shadow_fix(struct panel_device *panel,
		int id, int strength_percent, int luminance_threshold)
{
	...
	struct maptbl *gamma_maptbl;
	struct brightness_table *brt_tbl;
	int i, v, nr_luminance, r_delta, b_delta;
	...
	for (i = 0; i < nr_luminance; i++) {
		if (dim_info->dim_lut[i].luminance > luminance_threshold)
			continue;
		for (v = 0; v < dim_info->nr_tp && v < 12; v++) {
			r_delta = (precision_r_correction[v] * strength_percent) / 100;
			b_delta = (precision_b_correction[v] * strength_percent) / 100;
			dim_info->dim_lut[i].rgb_color_offset[v][RED] += r_delta;
			dim_info->dim_lut[i].rgb_color_offset[v][BLUE] += b_delta;
		}
	}
	...
	panel_info("%s applied precision fix strength:%d%% below %dnit\n",
			__func__, strength_percent, luminance_threshold);

	return 0;
}
EXPORT_SYMBOL(s6e3ha9_apply_precision_shadow_fix);

/*
 * s6e3ha9_apply_full_range_color_fix
 *
 * Unlike precision_shadow_fix (which reuses ONE brightness level's
 * measured data everywhere below a cutoff), this uses REAL measured
 * correction data from 9 different real brightness levels spanning
 * the panel's entire range (2nit to 380nit), and picks whichever
 * measured point is closest to each actual luminance entry. This
 * means dark UI gets dark-appropriate correction, bright content
 * gets bright-appropriate correction - no cutoff, no guessing,
 * no reusing data outside where it was actually measured.
 *
 * strength_percent: 0-100, scales all corrections together.
 */
static const int ref_nit[9] = { 2, 6, 14, 30, 60, 105, 172, 265, 380 };

static const int ref_r_corr[9][12] = {
	{ 0, 0, 0, -9, 8, 21, 25, 19, 16, 7, 3, 9 },
	{ 0, 0, 0, -9, 9, 19, 22, 13, 10, 3, 2, 3 },
	{ 0, 0, 0, -7, 9, 12, 11, 8, 5, 2, 2, -1 },
	{ 0, 0, 0, -4, 10, 13, 8, 3, 2, 1, 1, -2 },
	{ 0, 0, 0, 1, 11, 8, 1, -1, 2, -1, 2, -3 },
	{ 0, 0, 0, 4, 15, 5, 0, -1, 0, 1, 1, -4 },
	{ 0, 0, 0, 5, 11, 2, -1, 0, 0, 0, 0, -2 },
	{ 0, 0, 0, 7, 1, 1, -3, 0, 1, -1, 0, -1 },
	{ 0, 0, 0, 7, -2, -1, 1, -1, 0, 0, 0, -1 },
};

static const int ref_b_corr[9][12] = {
	{ 0, 0, 0, 6, 12, 15, 19, 12, 9, 4, 2, 6 },
	{ 0, 0, 0, 8, 12, 23, 17, 9, 6, 1, 1, 2 },
	{ 0, 0, 0, 10, 18, 20, 14, 8, 4, 1, 1, 0 },
	{ 0, 0, 0, 12, 23, 13, 11, 4, 1, 0, 1, -1 },
	{ 0, 0, 0, 23, 18, 3, 4, 2, 0, -1, 1, -2 },
	{ 0, 0, 0, 16, 16, 2, 3, 0, -1, 0, 1, -2 },
	{ 0, 0, 0, 14, 9, 1, 2, -1, -1, 0, 1, -3 },
	{ 0, 0, 0, 9, -1, -1, 0, -2, -1, 0, 0, -2 },
	{ 0, 0, 0, 1, -4, -1, 0, -1, 0, 1, -1, -1 },
};

static int nearest_ref_index(int luminance)
{
	int i, best = 0, best_dist = 100000, dist;

	for (i = 0; i < 9; i++) {
		dist = luminance - ref_nit[i];
		if (dist < 0)
			dist = -dist;
		if (dist < best_dist) {
			best_dist = dist;
...
int s6e3ha9_apply_full_range_color_fix(struct panel_device *panel,
		int id, int strength_percent)
{
	...
	struct maptbl *gamma_maptbl;
	struct brightness_table *brt_tbl;
	int i, v, nr_luminance, ref_idx, r_delta, b_delta;
	...
	/* Apply to EVERY luminance level, each using its own
	 * closest real measured reference point - no cutoff needed
	 */
	for (i = 0; i < nr_luminance; i++) {
		ref_idx = nearest_ref_index(dim_info->dim_lut[i].luminance);
		for (v = 0; v < dim_info->nr_tp && v < 12; v++) {
			r_delta = (ref_r_corr[ref_idx][v] * strength_percent) / 100;
			b_delta = (ref_b_corr[ref_idx][v] * strength_percent) / 100;
			dim_info->dim_lut[i].rgb_color_offset[v][RED] += r_delta;
			dim_info->dim_lut[i].rgb_color_offset[v][BLUE] += b_delta;
		}
	}
	...
	panel_info("%s applied full-range color fix strength:%d%%\n",
			__func__, strength_percent);

	return 0;
}
EXPORT_SYMBOL(s6e3ha9_apply_full_range_color_fix);

/*
 * s6e3ha9_apply_manual_gamma
 *
 * True manual control, not locked to any "correct" formula.
 * Unlike shadowRGB/precision_shadow_fix/color_fix (which all
 * calculate what SHOULD balance R/B against G), this lets you
 * push R, G, and B independently, in either direction, with no
 * cap other than what keeps the panel registers valid. Applied
 * in one of 3 zones so you're not stuck adjusting the whole
 * screen with one number.
 *
 * zone: 0 = dark   (luminance <= 60 nit)
 *       1 = mid    (60 < luminance <= 172 nit)
 *       2 = bright  (luminance > 172 nit)
 * r/g/b: offset applied to every tap in that zone, any direction.
 *   Real range that keeps registers valid is roughly -60 to +60.
 */
int s6e3ha9_apply_manual_gamma(struct panel_device *panel,
		int id, int zone, int r, int g, int b)
{
	...
	for (i = 0; i < nr_luminance; i++) {
		...
		if (zone == 0 && lum > 60)
			continue;
		if (zone == 1 && (lum <= 60 || lum > 172))
			continue;
		if (zone == 2 && lum <= 172)
			continue;

		for (v = 0; v < dim_info->nr_tp; v++) {
			dim_info->dim_lut[i].rgb_color_offset[v][RED] += r;
			dim_info->dim_lut[i].rgb_color_offset[v][GREEN] += g;
			dim_info->dim_lut[i].rgb_color_offset[v][BLUE] += b;
		}
	}
	...
	panel_info("%s applied manual gamma zone:%d r:%d g:%d b:%d\n",
			__func__, zone, r, g, b);

	return 0;
}
EXPORT_SYMBOL(s6e3ha9_apply_manual_gamma);
