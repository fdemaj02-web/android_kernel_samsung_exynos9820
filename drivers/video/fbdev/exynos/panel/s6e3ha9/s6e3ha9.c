...
static void copy_irc_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	/*
	 * IRC applies a per-channel, per-gray-zone multiplier sourced from
	 * this panel's factory calibration (register 0x95 / DIM_FLASH_IRC),
	 * weighted more heavily at low brightness. On a panel with no valid
	 * factory calibration (live MTP reads all-zero), that data is not
	 * trustworthy, and applying it injects a shadow-concentrated color
	 * skew instead of correcting one. Skip the interpolation and pass
	 * the base reference table through unmodified in that case - no
	 * correction is safer than a wrong one here.
	 */
