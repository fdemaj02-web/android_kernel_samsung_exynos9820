...
/*
 * manual_offset - live, honest RGB color + grayscale darkness offset
 * for testing gamma correction after an LCD/panel replacement.
 *
 * No pre-baked "measured" correction data. You supply deltas, look
 * at the real screen, iterate. Baseline (factory) values are captured
 * automatically the first time either function below is called, so
 * "reset" always has a real, exact state to restore - not a guess.
 *
 * Deltas are additive on top of whatever is currently applied.
 */

#define MANUAL_OFFSET_MAX_LUT 128
#define MANUAL_OFFSET_MAX_TP  16

static bool manual_offset_baseline_saved[MAX_PANEL_BL_SUBDEV];
static s32  manual_offset_baseline_rgb[MAX_PANEL_BL_SUBDEV][MANUAL_OFFSET_MAX_LUT][MANUAL_OFFSET_MAX_TP][MAX_COLOR];
static s32  manual_offset_baseline_gray[MAX_PANEL_BL_SUBDEV][MANUAL_OFFSET_MAX_LUT][MANUAL_OFFSET_MAX_TP];

static int manual_offset_capture_baseline_if_needed(struct dimming_info *dim_info, int id)
{
	int i, v, c, nr_lum, nr_tp;

	if (manual_offset_baseline_saved[id])
		return 0;

	nr_lum = dim_info->nr_luminance;
	nr_tp  = dim_info->nr_tp;

	if (nr_lum > MANUAL_OFFSET_MAX_LUT || nr_tp > MANUAL_OFFSET_MAX_TP) {
		panel_err("%s: table too large (lut=%d tp=%d), raise MANUAL_OFFSET_MAX_* and rebuild\n",
				__func__, nr_lum, nr_tp);
		return -ENOSPC;
	}

	for (i = 0; i < nr_lum; i++) {
		for (v = 0; v < nr_tp; v++) {
			manual_offset_baseline_gray[id][i][v] =
				dim_info->dim_lut[i].gray_scale_offset[v];
			for (c = 0; c < MAX_COLOR; c++)
				manual_offset_baseline_rgb[id][i][v][c] =
					dim_info->dim_lut[i].rgb_color_offset[v][c];
		}
	}

	manual_offset_baseline_saved[id] = true;
	panel_info("%s: baseline captured for subdev %d (lut=%d tp=%d)\n",
			__func__, id, nr_lum, nr_tp);
	return 0;
}

static int manual_offset_push(struct panel_device *panel, int id, struct dimming_info *dim_info)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct panel_bl_device *panel_bl = &panel->panel_bl;
	struct maptbl *gamma_maptbl;
	struct brightness_table *brt_tbl;
	int i;
	...
	gamma_maptbl = find_panel_maptbl_by_index(panel_data, GAMMA_MAPTBL);
	if (unlikely(!gamma_maptbl)) {
		panel_err("%s gamma_maptbl not found\n", __func__);
		return -EINVAL;
	}
	...
	panel_bl_set_brightness(panel_bl, id, 1);
	return 0;
}

/*
 * manual_offset_color - shift RGB color balance (ctbl-equivalent) for
 * tap points [tp_min, tp_max] (which GRAY TONE, black=0 to white=
 * nr_tp-1) within brightness levels [nit_min, nit_max] (which SCREEN
 * BRIGHTNESS setting). Two different axes - tp picks the tone, nit
 * picks which backlight level's curve gets touched. Pass tp_min=0,
 * tp_max=999 to hit every tap point like before (old behavior).
 * Negative red = less red, negative blue = less blue, etc.
 */
int manual_offset_color(struct panel_device *panel, int id,
		int nit_min, int nit_max, int tp_min, int tp_max,
		int dr, int dg, int db)
{
	...
	if (id >= MAX_PANEL_BL_SUBDEV) {
		panel_err("%s invalid id %d\n", __func__, id);
		return -EINVAL;
	}

	dim_info = (struct dimming_info *)panel_data->dim_info[id];
	if (unlikely(!dim_info)) {
		panel_err("%s dim_info not ready\n", __func__);
		return -ENODEV;
	}

	ret = manual_offset_capture_baseline_if_needed(dim_info, id);
	if (ret < 0)
		return ret;

	for (i = 0; i < dim_info->nr_luminance; i++) {
		lum = dim_info->dim_lut[i].luminance;
		if (lum < nit_min || lum > nit_max)
			continue;
		for (v = 0; v < dim_info->nr_tp; v++) {
			if (v < tp_min || v > tp_max)
				continue;
			dim_info->dim_lut[i].rgb_color_offset[v][RED]   += dr;
			dim_info->dim_lut[i].rgb_color_offset[v][GREEN] += dg;
			dim_info->dim_lut[i].rgb_color_offset[v][BLUE]  += db;
		}
	}

	ret = manual_offset_push(panel, id, dim_info);
	if (ret < 0)
		return ret;

	panel_info("%s applied r:%+d g:%+d b:%+d to tp %d-%d, %d-%dnit\n",
			__func__, dr, dg, db, tp_min, tp_max, nit_min, nit_max);
	return 0;
}
EXPORT_SYMBOL(manual_offset_color);

/*
 * manual_offset_gray - shift grayscale darkness (rtbl-equivalent) for
 * tap points [tp_min, tp_max] within brightness levels [nit_min,
 * nit_max]. Negative = darker at that tone, positive = lighter.
 * Pass tp_min=0, tp_max=999 for every tap point (old behavior).
 */
int manual_offset_gray(struct panel_device *panel, int id,
		int nit_min, int nit_max, int tp_min, int tp_max, int delta)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int i, v, lum, ret;
	...
	for (i = 0; i < dim_info->nr_luminance; i++) {
		...
		for (v = 0; v < dim_info->nr_tp; v++) {
			if (v < tp_min || v > tp_max)
				continue;
			dim_info->dim_lut[i].gray_scale_offset[v] += delta;
		}
	}
	...
	panel_info("%s applied delta:%+d to tp %d-%d, %d-%dnit\n",
			__func__, delta, tp_min, tp_max, nit_min, nit_max);
	return 0;
}
EXPORT_SYMBOL(manual_offset_gray);

/*
 * manual_offset_reset - restore the real captured factory baseline
 * for both color and gray, exactly (not by negating deltas).
 */
int manual_offset_reset(struct panel_device *panel, int id)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int i, v, c, ret;
	...
	if (!manual_offset_baseline_saved[id]) {
		panel_err("%s no baseline captured yet - nothing to reset\n", __func__);
		return -ENODATA;
	}
	...
	panel_info("%s restored factory baseline\n", __func__);
	return 0;
}
EXPORT_SYMBOL(manual_offset_reset);

/*
 * manual_offset_gray_array / manual_offset_color_array - set every
 * tap point's offset explicitly in one call, instead of one uniform
 * delta across a range. Index 0 = deepest black, increasing toward
 * white. These SET the absolute offset value (not additive), so
 * calling twice with the same array is idempotent. Same baseline
 * capture/reset as the other manual_offset functions - "reset" on
 * either manual_offset or manual_offset_gray restores everything.
 */
int manual_offset_gray_array(struct panel_device *panel, int id,
		int nit_min, int nit_max, s32 *deltas, int nr_deltas)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int i, v, lum, ret;
	...
	for (i = 0; i < dim_info->nr_luminance; i++) {
		...
		if (lum < nit_min || lum > nit_max)
			continue;
		for (v = 0; v < dim_info->nr_tp && v < nr_deltas; v++)
			dim_info->dim_lut[i].gray_scale_offset[v] = deltas[v];
	}
	...
	panel_info("%s applied %d per-tap deltas, %d-%dnit\n",
			__func__, nr_deltas, nit_min, nit_max);
	return 0;
}
EXPORT_SYMBOL(manual_offset_gray_array);

int manual_offset_color_array(struct panel_device *panel, int id,
		int nit_min, int nit_max, s32 *rgb, int nr_rgb)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int i, v, lum, ret;
	...
	for (i = 0; i < dim_info->nr_luminance; i++) {
		...
		if (lum < nit_min || lum > nit_max)
			continue;
		for (v = 0; v < dim_info->nr_tp && (v * 3 + 2) < nr_rgb; v++) {
			dim_info->dim_lut[i].rgb_color_offset[v][RED]   = rgb[v * 3 + 0];
			dim_info->dim_lut[i].rgb_color_offset[v][GREEN] = rgb[v * 3 + 1];
			dim_info->dim_lut[i].rgb_color_offset[v][BLUE]  = rgb[v * 3 + 2];
		}
	}
	...
	panel_info("%s applied %d per-tap RGB triplets, %d-%dnit\n",
			__func__, nr_rgb / 3, nit_min, nit_max);
	return 0;
}
EXPORT_SYMBOL(manual_offset_color_array);

/*
 * manual_offset_status - read-only dump of the REAL captured factory
 * baseline for one LUT (brightness/nit level), so an array-mode write
 * (which SETS absolute values, not additive) can be built from real
 * numbers instead of guessing 0 for tap points you don't mean to touch.
 *
 * Select which LUT to inspect by writing an index to manual_offset_status_lut
 * first (0 = dimmest table), then read manual_offset_status.
 */
static int manual_offset_status_lut_sel;

int manual_offset_status_lut_store_impl(struct panel_device *panel, int id, int lut)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	...
	if (unlikely(!dim_info))
		return -ENODEV;
	if (lut < 0 || lut >= dim_info->nr_luminance)
		return -EINVAL;

	manual_offset_status_lut_sel = lut;
	return 0;
}
EXPORT_SYMBOL(manual_offset_status_lut_store_impl);

int manual_offset_status_show_impl(struct panel_device *panel, int id, char *buf, size_t bufsize)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int v, nr_tp, len = 0;
	...
	manual_offset_capture_baseline_if_needed(dim_info, id);
	nr_tp = dim_info->nr_tp;

	len += scnprintf(buf + len, bufsize - len,
		"lut=%d (of %d) nr_tp=%d  -- factory baseline, tap: gray  R G B\n",
		manual_offset_status_lut_sel, dim_info->nr_luminance, nr_tp);

	for (v = 0; v < nr_tp && len < bufsize - 64; v++) {
		len += scnprintf(buf + len, bufsize - len,
			"tp%-2d: %5d   %5d %5d %5d\n", v,
			manual_offset_baseline_gray[id][manual_offset_status_lut_sel][v],
			manual_offset_baseline_rgb[id][manual_offset_status_lut_sel][v][0],
			manual_offset_baseline_rgb[id][manual_offset_status_lut_sel][v][1],
			manual_offset_baseline_rgb[id][manual_offset_status_lut_sel][v][2]);
	}
	return len;
}
EXPORT_SYMBOL(manual_offset_status_show_impl);

/*
 * manual_offset_tp_cal / manual_offset_vt - deeper level than everything
 * above. These touch the panel's actual MTP calibration inputs (tp[].offset,
 * vt_voltage[]) that generate_gamma_table() uses to BUILD the per-luminance
 * tables from scratch - not a layer added on top of them. More powerful,
 * more foundational, and touches every brightness level at once since it
 * changes the source data the whole table is generated from.
 *
 * Real baseline captured on first use, same honesty guarantee as the
 * rest of this file - reset always restores your actual factory numbers.
 */
static bool manual_offset_tpvt_baseline_saved[MAX_PANEL_BL_SUBDEV];
static s32  manual_offset_tp_baseline[MAX_PANEL_BL_SUBDEV][MANUAL_OFFSET_MAX_TP][MAX_COLOR];
static u32  manual_offset_vt_baseline[MAX_PANEL_BL_SUBDEV][16];

static int manual_offset_tpvt_capture_baseline_if_needed(struct dimming_info *dim_info, int id)
{
	int v, c;

	if (manual_offset_tpvt_baseline_saved[id])
		return 0;

	if (dim_info->nr_tp > MANUAL_OFFSET_MAX_TP) {
		panel_err("%s: nr_tp %d too large, raise MANUAL_OFFSET_MAX_TP\n",
				__func__, dim_info->nr_tp);
		return -ENOSPC;
	}

	for (v = 0; v < dim_info->nr_tp; v++)
		for (c = 0; c < MAX_COLOR; c++)
			manual_offset_tp_baseline[id][v][c] = dim_info->tp[v].offset[c];

	for (v = 0; v < 16; v++)
		manual_offset_vt_baseline[id][v] = dim_info->vt_voltage[v];

	manual_offset_tpvt_baseline_saved[id] = true;
	panel_info("%s: tp/vt baseline captured for subdev %d\n", __func__, id);
	return 0;
}

/* echo "TP_INDEX DR DG DB" > manual_offset_tp_cal  -- additive, real MTP offset */
int manual_offset_tp_cal(struct panel_device *panel, int id, int tp_idx, int dr, int dg, int db)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int ret;
	...
	if (unlikely(!dim_info))
		return -ENODEV;
	if (tp_idx < 0 || tp_idx >= dim_info->nr_tp)
		return -EINVAL;

	ret = manual_offset_tpvt_capture_baseline_if_needed(dim_info, id);
	if (ret < 0)
		return ret;

	dim_info->tp[tp_idx].offset[RED]   += dr;
	dim_info->tp[tp_idx].offset[GREEN] += dg;
	dim_info->tp[tp_idx].offset[BLUE]  += db;

	ret = manual_offset_push(panel, id, dim_info);
	if (ret < 0)
		return ret;

	panel_info("%s: tp%d offset now R=%d G=%d B=%d\n", __func__, tp_idx,
			dim_info->tp[tp_idx].offset[RED],
			dim_info->tp[tp_idx].offset[GREEN],
			dim_info->tp[tp_idx].offset[BLUE]);
	return 0;
}
EXPORT_SYMBOL(manual_offset_tp_cal);

/* echo "VT_INDEX DELTA" > manual_offset_vt  -- additive, real black-point voltage ref */
int manual_offset_vt(struct panel_device *panel, int id, int vt_idx, int delta)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int ret;
	...
	if ((s32)dim_info->vt_voltage[vt_idx] + delta < 0) {
		panel_err("%s: vt%d would go negative, refusing\n", __func__, vt_idx);
		return -EINVAL;
	}
	dim_info->vt_voltage[vt_idx] += delta;

	ret = manual_offset_push(panel, id, dim_info);
	if (ret < 0)
		return ret;

	panel_info("%s: vt%d now %u\n", __func__, vt_idx, dim_info->vt_voltage[vt_idx]);
	return 0;
}
EXPORT_SYMBOL(manual_offset_vt);

int manual_offset_tpvt_reset(struct panel_device *panel, int id)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int v, c, ret;
	...
	if (!manual_offset_tpvt_baseline_saved[id])
		return 0; /* nothing touched yet, nothing to restore */

	for (v = 0; v < dim_info->nr_tp; v++)
		for (c = 0; c < MAX_COLOR; c++)
			dim_info->tp[v].offset[c] = manual_offset_tp_baseline[id][v][c];

	for (v = 0; v < 16; v++)
		dim_info->vt_voltage[v] = manual_offset_vt_baseline[id][v];

	ret = manual_offset_push(panel, id, dim_info);
	if (ret < 0)
		return ret;

	panel_info("%s: tp/vt restored to real factory baseline\n", __func__);
	return 0;
}
EXPORT_SYMBOL(manual_offset_tpvt_reset);

/* read-only: dump every tp[].offset and vt_voltage[], current live values */
int manual_offset_tpvt_status_show_impl(struct panel_device *panel, int id, char *buf, size_t bufsize)
{
	struct panel_info *panel_data = &panel->panel_data;
	struct dimming_info *dim_info;
	int v, len = 0;
	...
	len += scnprintf(buf + len, bufsize - len,
		"-- tp[].offset (real MTP calibration, per tap point) --\n"
		"tp   R      G      B\n");
	for (v = 0; v < dim_info->nr_tp && len < bufsize - 64; v++)
		len += scnprintf(buf + len, bufsize - len, "%-4d %-6d %-6d %-6d\n",
				v, dim_info->tp[v].offset[RED],
				dim_info->tp[v].offset[GREEN],
				dim_info->tp[v].offset[BLUE]);

	len += scnprintf(buf + len, bufsize - len,
		"-- vt_voltage[] (black-point voltage reference) --\n");
	for (v = 0; v < 16 && len < bufsize - 32; v++)
		len += scnprintf(buf + len, bufsize - len, "vt%-2d %u\n",
				v, dim_info->vt_voltage[v]);

	return len;
}
EXPORT_SYMBOL(manual_offset_tpvt_status_show_impl);

/*
 * manual_offset_irc - persistent per-tap delta applied to the IRC
 * (low-brightness shadow compensation) table on every recompute,
 * whether or not factory IRC calibration exists. Index 0 = deepest
 * shadow tap point. Survives brightness changes (re-applied every
 * time copy_irc_maptbl runs), unlike raw ref_tbl edits which would
 * get overwritten on the next brightness step.
 */
s32 manual_offset_irc_delta[MANUAL_OFFSET_MAX_TP];
bool manual_offset_irc_active;

int manual_offset_irc_set(s32 *deltas, int nr_deltas)
{
	int v;

	memset(manual_offset_irc_delta, 0, sizeof(manual_offset_irc_delta));
	for (v = 0; v < MANUAL_OFFSET_MAX_TP && v < nr_deltas; v++)
		manual_offset_irc_delta[v] = deltas[v];
	manual_offset_irc_active = true;
	pr_info("%s applied %d per-tap IRC deltas\n", __func__, nr_deltas);
	return 0;
}
EXPORT_SYMBOL(manual_offset_irc_set);

int manual_offset_irc_reset(void)
{
	memset(manual_offset_irc_delta, 0, sizeof(manual_offset_irc_delta));
	manual_offset_irc_active = false;
	pr_info("%s cleared\n", __func__);
	return 0;
}
EXPORT_SYMBOL(manual_offset_irc_reset);
...
static int init_gamma_table(struct maptbl *tbl)
{
	...
	struct panel_info *panel_data = &panel->panel_data;
	int __i;
	bool no_factory_cal = true;

	/*
	 * Independent, self-contained check: don't trust cur_dim_type alone
	 * to have correctly routed us away from raw flash gamma. On a panel
	 * with no factory calibration, generate_gamma_table_using_flash()
	 * copies unvalidated bytes from blank/garbage flash for every single
	 * brightness step (including all shadow/low-gray steps), producing
	 * non-monotonic, banded, "unsmooth" gamma between adjacent gray
	 * levels. Verify the live MTP read ourselves here and force the
	 * algorithmic LUT-based path (which produces a smooth, monotonic
	 * curve) whenever there's no real per-unit calibration, regardless
	 * of what cur_dim_type says.
	 */
	for (__i = 0; __i < sizeof(panel->dim_flash_result.mtp_reg); __i++) {
		if (panel->dim_flash_result.mtp_reg[__i] != 0) {
			no_factory_cal = false;
			break;
		}
	}

	if (no_factory_cal) {
		pr_warn("%s, no factory calibration - forcing smooth LUT-based gamma (not raw flash)\n",
				__func__);
...
static void copy_irc_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	if (manual_offset_irc_active) {
		int __v, __nr_tp;

		__nr_tp = irc_info->total_len / MAX_COLOR;
		if (__nr_tp > MANUAL_OFFSET_MAX_TP)
			__nr_tp = MANUAL_OFFSET_MAX_TP;
		for (__v = 0; __v < __nr_tp; __v++) {
			int __base = __v * MAX_COLOR;
			int __c;
			for (__c = 0; __c < MAX_COLOR; __c++) {
				int __val = (int)irc_info->ref_tbl[__base + __c] +
						manual_offset_irc_delta[__v];
				if (__val < 0)
					__val = 0;
				if (__val > 255)
					__val = 255;
				irc_info->ref_tbl[__base + __c] = (u8)__val;
			}
		}
	}

	{
		...
		if (no_factory_cal) {
			pr_warn("%s, no factory IRC calibration - bypassing IRC interpolation\n", __func__);
			memcpy(dst, irc_info->ref_tbl, irc_info->total_len);
			return;
		}
	}
...
static int init_maptbl_from_flash(struct maptbl *tbl, enum dim_flash_items item)
{
	...
	if (!panel_resource_initialized(panel_data,
		...
		pr_warn("%s resource(%s) not initialized\n",
				__func__, s6e3ha9_dim_flash_info[item].name);
